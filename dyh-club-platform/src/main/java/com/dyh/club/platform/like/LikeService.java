...
public class LikeService {
    ...
    static final String TOGGLE_SCRIPT =
            "local old=redis.call('hget',KEYS[1],ARGV[1]); " +
            "local ver=redis.call('hget',KEYS[3],ARGV[2]); " +
            "local cnt=redis.call('get',KEYS[2]); " +
            "if not cnt then cnt=ARGV[7]; redis.call('set',KEYS[2],cnt) end; " +
            "if not ver or tonumber(ver)<tonumber(ARGV[6]) then " +
            "if ver and old and old~=ARGV[5] then " +
            "if ARGV[5]=='1' then cnt=redis.call('incr',KEYS[2]) " +
            "elseif tonumber(cnt)>0 then cnt=redis.call('decr',KEYS[2]) end end; " +
            "old=ARGV[5]; ver=ARGV[6]; " +
            "redis.call('hset',KEYS[1],ARGV[1],old); redis.call('hset',KEYS[3],ARGV[2],ver); " +
            "elseif not old then old=ARGV[5]; redis.call('hset',KEYS[1],ARGV[1],old) end; " +
            "local incoming=tonumber(ARGV[8]); local current=tonumber(ver); " +
            "if incoming<=current then return {0,current,tonumber(cnt),tonumber(old)} end; " +
            "redis.call('hset',KEYS[3],ARGV[2],incoming); " +
            "if old~=ARGV[3] then redis.call('hset',KEYS[1],ARGV[1],ARGV[3]); " +
            "if ARGV[3]=='1' then cnt=redis.call('incr',KEYS[2]) " +
            "elseif tonumber(cnt)>0 then cnt=redis.call('decr',KEYS[2]) end end; " +
            "redis.call('hset',KEYS[4],ARGV[2],ARGV[3]..'|'..incoming); " +
            "redis.call('zadd',KEYS[5],ARGV[4],ARGV[2]); " +
            "return {1,incoming,tonumber(cnt),tonumber(ARGV[3])}";
    private static final DefaultRedisScript<List> TOGGLE=new DefaultRedisScript<>(TOGGLE_SCRIPT,List.class);
    ...
    public Map<String,Object> set(long questionId,long userId,boolean liked){
        ...
        LikeEvent event;long count;boolean actualLiked=liked;
        if(redis!=null){try{
            ...
            if(result==null||result.size()<4)throw new IllegalStateException("点赞 Lua 返回值异常");long changed=((Number)result.get(0)).longValue();long version=((Number)result.get(1)).longValue();count=((Number)result.get(2)).longValue();actualLiked=((Number)result.get(3)).longValue()==1;event=new LikeEvent(questionId,userId,actualLiked,version);if(changed==1)publishOrPersist(event);
        }catch(Exception error){log.warn("Redis 点赞链路失败，降级为数据库条件写入",error);event=new LikeEvent(questionId,userId,liked,allocatedVersion);requireApplied(event);count=mysqlCount(questionId);actualLiked=liked;}}
        else{event=new LikeEvent(questionId,userId,liked,allocatedVersion);requireApplied(event);count=mysqlCount(questionId);}
        Map<String,Object> response=new LinkedHashMap<>();response.put("questionId",questionId);response.put("liked",actualLiked);response.put("count",count);response.put("version",event.version);return response;
    }
