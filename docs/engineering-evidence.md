...
| 点赞最终一致 | MySQL 单调版本分配、Redis Lua、RocketMQ、pending 时长、补偿与受控恢复 | 数据库高版本/Redis 丢失、重复/乱序、降级状态测试 |
| 搜索同步与恢复 | ES 集群节点轮询、IK、500 条 Bulk、逐项失败重试、Canal checkpoint、近期校验、增量重放与别名切换 | IK 映射、节点列表、Bulk 失败项、批量上限和 MySQL 降级测试 |
...
| 生产型高可用证据 | `deploy/ha` 的 Redis Sentinel、RocketMQ 同步主从、Nacos 三节点、ES 三节点 | 独立 Compose 静态校验；不冒充本地单节点运行结果 |
