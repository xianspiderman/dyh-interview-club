package com.dyh.club.practice.server.service.impl;

import com.alibaba.fastjson.JSON;
import com.google.gson.Gson;
import com.dyh.club.practice.api.common.PageInfo;
import com.dyh.club.practice.api.common.PageResult;
import com.dyh.club.practice.api.enums.CompleteStatusEnum;
import com.dyh.club.practice.api.enums.IsDeletedFlagEnum;
import com.dyh.club.practice.api.enums.SubjectInfoTypeEnum;
import com.dyh.club.practice.api.req.GetPracticeSubjectsReq;
import com.dyh.club.practice.api.req.GetUnCompletePracticeReq;
import com.dyh.club.practice.api.vo.*;
import com.dyh.club.practice.server.dao.*;
import com.dyh.club.practice.server.entity.dto.CategoryDTO;
import com.dyh.club.practice.server.entity.dto.PracticeSetDTO;
import com.dyh.club.practice.server.entity.dto.PracticeSubjectDTO;
import com.dyh.club.practice.server.entity.po.*;
import com.dyh.club.practice.server.service.PracticeSetService;
import com.dyh.club.practice.server.util.DateUtils;
import com.dyh.club.practice.server.util.LoginUtil;
...
public class  PracticeSetServiceImpl implements PracticeSetService {
    ...
    public List<SpecialPracticeVO> getSpecialPracticeContent() {
        ...
        List<Integer> subjectTypeList = new LinkedList<>();//专项练习只刷单选、多选和判断，这是题目的类型的列表
        ...
        List<PrimaryCategoryPO> poList = subjectCategoryDao.getPrimaryCategory(categoryDTO);//获取所有的大类
        if (CollectionUtils.isEmpty(poList)) {
            return specialPracticeVOList;
        }
        // 下面开始组装，现在知道所有的大类(poList),现在要知道大类的名儿以及大类下边的分类
        poList.forEach(primaryCategoryPO -> {
            ...
            specialPracticeVO.setPrimaryCategoryName(categoryPO.getCategoryName());//得到分类名
            CategoryDTO categoryDTOTemp = new CategoryDTO();//查询大类下的分类
            categoryDTOTemp.setCategoryType(2);//2是只查找小类
            categoryDTOTemp.setParentId(primaryCategoryPO.getParentId());
            List<CategoryPO> smallPoList = subjectCategoryDao.selectList(categoryDTOTemp);//目的就是拿小类
            ...
            smallPoList.forEach(smallPo -> {
                List<SpecialPracticeLabelVO> labelVOList = getLabelVOList(smallPo.getId(), subjectTypeList);//目的根据小类就是拿标签
