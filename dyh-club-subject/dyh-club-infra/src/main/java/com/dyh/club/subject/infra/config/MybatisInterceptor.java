package com.dyh.club.subject.infra.config;
...
public class MybatisInterceptor implements Interceptor {
    ...
    public Object intercept(Invocation invocation) throws Throwable {
        ...
        SqlCommandType sqlCommandType = mappedStatement.getSqlCommandType();//得到Sql的指令类型(get、set、update等等)
        Object parameter = invocation.getArgs()[1];//也就是createBy,createTime等公共字段
        if (parameter == null) {
            return invocation.proceed();//直接放行
        }
        //获取当前登录用户的id
        ...
        if (SqlCommandType.INSERT == sqlCommandType || SqlCommandType.UPDATE == sqlCommandType) {
            replaceEntityProperty(parameter, loginId, sqlCommandType);//只拦截处理插入和更新
        }
        return invocation.proceed();
    }

    private void replaceEntityProperty(Object parameter, String loginId, SqlCommandType sqlCommandType) {
        if (parameter instanceof Map) {//map形式的处理
    ...
    private void dealInsert(Object parameter, String loginId) {
        ...
        for (Field field : fields) {
            try {
                field.setAccessible(true);
                Object o = field.get(parameter);
                if (Objects.nonNull(o)) {//只处理（createBy,createTime等）为空的情况，不为空不处理
                    field.setAccessible(false);
                    continue;
                }
    ...
    @Override//这是继承Interceptor接口自动重写的
    public Object plugin(Object target) {
        return Plugin.wrap(target, this);
    }

    @Override//这是继承Interceptor接口自动重写的，没动
